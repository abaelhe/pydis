...
(CONNECT, DISCONNECT, EVENT, ACK, ERROR, BINARY_EVENT, BINARY_ACK) = (0, 1, 2, 3, 4, 5, 6)
packet_names = ['CONNECT', 'DISCONNECT', 'EVENT', 'ACK', 'ERROR', 'BINARY_EVENT', 'BINARY_ACK']
...
class Packet(object):
    ...
    def __init__(self, packet_type=EVENT, data=None, namespace=None, id=None, binary=None, encoded_packet=None):
    ...
    def _reconstruct_binary_internal(self, data, attachments):
        if isinstance(data, list):
            return [self._reconstruct_binary_internal(item, attachments) for item in data]
        elif isinstance(data, dict):
            if data.get('_placeholder') and 'num' in data:
                return attachments[data['num']]
            else:
                return {k: self._reconstruct_binary_internal(v, attachments) for k, v in six.iteritems(data)}
        else:
            return data
    ...
    def _data_is_binary(self, data):
        ...
        if isinstance(data, six.binary_type):
            ...
            return reduce(lambda a, b: a or b, [self._data_is_binary(item) for item in data], False)
        elif isinstance(data, dict):
            return reduce(lambda a, b: a or b, [self._data_is_binary(item) for item in six.itervalues(data)], False)
        else:
            return False
