...
class Socket(object):
    ...
    def create_queue(self):
        return getattr(self.server._async['queue'], self.server._async['queue_class'])()
    ...
    def _websocket_handler(self, ws):
        ...
        if self.connected:
            ...
            pkt = ws.wait()
            if pkt != packet.Packet(packet.PING, data=six.text_type('probe')).encode(always_bytes=False):
                self.server.logger.info('%s: Failed websocket upgrade, no PING packet', self.sid)
                return []
            ws.send(packet.Packet(packet.PONG, data=six.text_type('probe')).encode(always_bytes=False))
            self.send(packet.Packet(packet.NOOP))
            ...
            if decoded_pkt.packet_type != packet.UPGRADE:
                ...
                self.server.logger.info(
                    ('%s: Failed websocket upgrade, expected UPGRADE packet, received %s instead.'), self.sid, pkt)
                return []
            self.upgraded = True
        else:
            self.connected = True
