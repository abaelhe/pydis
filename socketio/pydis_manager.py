...
class RedisManager(PubSubManager):  # pragma: no cover
    ...
    def __init__(self, url='redis://localhost:6379/0', channel='socketio', write_only=False, logger=None):
        if redis is None:
            raise RuntimeError('Redis package is not installed (Run "pip install redis" in your virtualenv).')
        self.redis_url = url
        self._redis_connect()
        super(RedisManager, self).__init__(channel=channel, write_only=write_only, logger=logger)
    ...
    def _publish(self, data):
        ...
        while True:
            try:
                ...
                if retry:
                    logger.error('Error cannot publish to redis... retrying')
                    retry = False
